defmodule ConnectFour.Contenders.R2 do
  use GenServer
  require IEx
  ...
  def handle_call({:move, board}, _from, state) do
    ...
    first_move = first_move(board, 1)
    
    state
    |> IO.inspect

    {:reply, (winning_move || blocking_move || first_move ||  random_column), state}
  end

  def first_move(board,contender) do
  end
